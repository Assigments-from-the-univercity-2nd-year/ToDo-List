...
class TodoPartDataDaoText {
    ...
    private suspend fun addTodoPartDatasToDatabase() {
        dao.addTodoPart(TodoPartDbModel("a content", 1, 1L, false))
        dao.addTodoPart(TodoPartDbModel("a content", 2, 1L, false))
        dao.addTodoPart(TodoPartDbModel("a content", 1, 2L, false))
        dao.addTodoPart(TodoPartDbModel("a content", 3, 1L, false))
        dao.addTodoPart(TodoPartDbModel("a content", 1, 3L, false))
        dao.addTodoPart(TodoPartDbModel("a content", 5, 1L, false))
    }
    ...
    fun getTodoPartDatasOfTask() = runBlocking {
        ...
        val id = 1L
        val img = dao.getTodoPartOfTask(id).asLiveData().getOrAwaitValue()
